...
def dialog_data_bundle_form(db: DataBundle = None) -> None:
    ...
    new_technology = col_techno.selectbox(
        'Endpoint technology ❗️',
        options=ENDPOINT_TECHNOLOGIES_STR,
        index=ENDPOINT_TECHNOLOGIES_STR.index(db.endpoint.technology_name) if db else None,
        help=help_text("data_bundle_form.endpoint_technology") or "[What are the supported SPARQL endpoint technologies?](/documentation#what-are-the-supported-sparql-endpoint-technologies)"
    )
    new_url = col_url.text_input(
        'Endpoint URL ❗️',
        value=db.endpoint.url if db else '',
        help=help_text("data_bundle_form.endpoint_url") or "[What is a SPARQL endpoint?](/documentation#what-is-a-sparql-endpoint)"
    )

    # SPARQL endpoint credentials
    col_username, col_password = st.columns([1, 1])
    new_username = col_username.text_input(
        'Endpoint username',
        value=db.endpoint.username if db else '',
        help=help_text("data_bundle_form.endpoint_username") or "[Where do I find my SPARQL endpoint username and password?](/documentation#in-the-data-bundle-creation-where-do-i-find-my-sparql-endpoint-username-and-password)"
    )
    new_password = col_password.text_input(
        'Endpoint password',
        value=db.endpoint.password if db else '',
        type='password',
        help=help_text("data_bundle_form.endpoint_password") or "[Where do I find my SPARQL endpoint username and password?](/documentation#in-the-data-bundle-creation-where-do-i-find-my-sparql-endpoint-username-and-password)"
    )
