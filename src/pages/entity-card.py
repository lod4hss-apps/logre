...
def render_entity_details(data_bundle, entity_uri: str) -> None:
    """Render the entity card for the currently selected entity."""
...
try:
    ...
    if not data_bundle:
        ...
        model_ready = data_bundle.has_model_definitions()
        pending_action = state.pop_pending_entity_action()
        if pending_action == 'find' and model_ready:
            dialog_find_entity()
        elif pending_action == 'create' and model_ready:
            dialog_entity_creation()

        if not entity_uri:
            st.warning('No Entity URI provided')
        else:
            render_entity_details(data_bundle, entity_uri)
