            ...
            with st.container(horizontal=True, horizontal_alignment='center'):
                if st.button(f'Upload Turtle file into the {data_type.upper()} named graph', type='primary', icon=':material/upload:'):
                    def upload_turtle(turtle_content: str) -> None:
                        if data_type == "Data": graph = data_bundle.data
                        if data_type == "Model": graph = data_bundle.model
                        if data_type == "Metadata": graph = data_bundle.metadata
...
with st.expander('Export'):
    ...
    # Export as turtle: need a choice of what to download: data, model or metadata
        ...
        with st.container(horizontal=True, horizontal_alignment='center'):
            ...
            if data_type == 'Data': graph = data_bundle.data
            elif data_type == 'Model': graph = data_bundle.model
            elif data_type == 'Metadata': graph = data_bundle.metadata
...
with st.container(horizontal=True, horizontal_alignment='right'):
    st.markdown("More on models and SHACL files in the [Documentation FAQ](/documentation#what-is-shacl)", width='content')