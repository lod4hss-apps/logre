...
try:
    ...
    with row_container.container(horizontal=True):
        ...
        not_value_classes = [
            c for c in data_bundle.model.classes if c.class_uri != "rdfs:Datatype"
        ]
        classes_labels = [c.get_text() for c in not_value_classes]
        class_label = st.selectbox(
            "Get entity table of class:", options=classes_labels, index=None, width=200
        )
    ...
    if class_label:
        ...
        with row_container.container(horizontal=True, horizontal_alignment="center"):
            sort_col = st.selectbox(
                ...
                columns,
                index=None,
                on_change=lambda: state.data_table_set_page(selected_class.uri, 1),
                width=200,
            )
        ...
        df_instances = data_bundle.get_data_table(
            selected_class, limit, offset, sort_col, sort_way, filter_col, filter_value
        )
        ...
        if len(df_instances) > 0:
            df_instances.columns = (
                ["URI"]
                + data_bundle.get_data_table_columns_names(selected_class)
