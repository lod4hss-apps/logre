...
def query(
    endpoint_url: str, query_text: str, username: str, password: str
) -> Optional[dict]:
    headers = {
...
def list_contexts(
    endpoint_url: str, username: str, password: str
) -> List[Tuple[str, int]]:
...
def ensure_data_graph(bundle: dict, prefixes: Dict[str, str]) -> bool:
    ...
    if candidate:
        ...
        print(
            f"[autoconfigure] Data graph for '{bundle.get('name', endpoint_url)}' set to {bundle['graph_data_uri']}"
        )
        return True
    ...
    if default_graph_has_data(endpoint_url, username, password):
        ...
        print(
            f"[autoconfigure] Data graph for '{bundle.get('name', endpoint_url)}' set to default graph"
        )
        return True
...
def autoconfigure_config(path: Path) -> bool:
    try:
